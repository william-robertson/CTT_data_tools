...
##
##  Error test attempt #1 using calibration from april and error test data from telemetry in June and July
##
...
##
##    Files Needed
##        Files Needed -- note that we hav renamed TestInfo.csv to fit each error test --error_testUTM.csv
...
outpath <- "C:/Users/wor5079/OneDrive - The Pennsylvania State University/MASTERS/CTT/CTT_data_tools/EcoEvo_PaxtonOptimization_example/errortest_output/"
...
test.info <- read.csv("error_testUTM.csv", header = T)
str(test.info) # check that data imported properly

#need to combine beep data into one RDS file --- this may only be specific to this script/ round
#beep.dat <- readRDS("BeepData_Example.rds") 
#str(beep.dat) # check that data imported properly

data1 <- readRDS("beep_data_2024-06-09_2024-06-13.rds")
data2 <- readRDS("beep_data_2024-06-14_2024-06-27.rds")
data3 <- readRDS("beep_data_2024-06-27_2024-07-08.rds")
